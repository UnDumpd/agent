...
func init() {
	DefaultRegistry.Register("rowcount", RowcountRunner{})
}

// defaultMaxDropPct — SPEC §6: fail when the row count drops more than 10%
// against the last known good run, unless max_drop_pct overrides it.
// A YAML max_drop_pct of 0 is indistinguishable from "unset" and gets the
// default; use a small positive value to forbid any drop.
const defaultMaxDropPct = 10.0

type RowcountRunner struct{}

func (r RowcountRunner) Run(ctx context.Context, checkCtx Context, cfg config.CheckConfig) (models.CheckResult, error) {
