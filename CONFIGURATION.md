...
| `uri` | string | for `s3` | Either a full object key (`s3://bucket/path/file.dump`) or a **prefix** ending in `/` (`s3://bucket/path/`). With a prefix, the agent lists the objects under it and picks the one with the most recent `LastModified` — i.e. "always test the newest backup". The exception is a prefix that turns out to be a `mongodump` directory: there the whole prefix is one dump and every direct child is downloaded — see "MongoDB dumps" below. |
...
| `pattern` | string (glob) | no | Narrows S3 prefix selection or local directory selection to entries whose **basename** matches the glob, e.g. `*.dump`. It is invalid with a full S3 object key, an exact local file, or a `mongodump` directory/prefix — there the whole thing is already one dump, so there is nothing to select inside it. |
